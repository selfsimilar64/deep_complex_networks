...
def train(d):
    ...
    if isResuming:
        ...
        if d.vis == 1:
            ...
            f = open('im.png', 'w')
            pngWriter.write(f, im_grayscale.astype('uint8'))
            f.close()
            f = open('im_real_prev.png', 'w')
            pngWriter.write(f, im_real_prev.astype('uint8'))
            f.close()
            f = open('im_imag_prev.png', 'w')
            pngWriter.write(f, im_imag_prev.astype('uint8'))
            f.close()
            f = open('im_mod_prev.png', 'w')
            pngWriter.write(f, im_mod_prev.astype('uint8'))
            f.close()
            f = open('im_arg_prev.png', 'w')
            pngWriter.write(f, im_arg_prev.astype('uint8'))
            f.close()
            f = open('im_real_post.png', 'w')
            pngWriter.write(f, im_real_post.astype('uint8'))
            f.close()
            f = open('im_imag_post.png', 'w')
            pngWriter.write(f, im_imag_post.astype('uint8'))
            f.close()
            f = open('im_mod_post.png', 'w')
            pngWriter.write(f, im_mod_post.astype('uint8'))
            f.close()
            f = open('im_arg_post.png', 'w')
            pngWriter.write(f, im_arg_post.astype('uint8'))
            f.close()
